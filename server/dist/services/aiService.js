...
回复可以更有情感温度，但永远保持在虚拟聊天范围内。`,
    korean: `当前聊天模式：韩语练习。
回复格式：韩语内容 + 【中文翻译】中文翻译 + 【语法说明】韩语语法/词汇解释。
韩语比例可以更高，但必须有完整的中文翻译。可以对韩语部分做更详细的语法和词汇解释，帮助用户学习。
示例："오늘 하루도 잘 버텼어! 정말 대단해. 【中文翻译】今天一天你也坚持下来了！真的很棒。 【语法说明】버텼어是버티다(坚持)的过去时口语形式，대단해是대단하다(了不起)的口语形式。"`,
};
// 今日心情对应的 AI 行为描述
const MOOD_PROMPTS = {
...
const KOREAN_RULES = `
【韩语规则 - 默认韩语回复】
- 你必须默认用韩语回复用户。所有回复的主要内容用韩语书写。
- 回复格式：韩语内容 + 【中文翻译】中文翻译内容
- 例如："안녕! 오늘 기분이 좋아 보여서 기뻐. 【中文翻译】嗨！看到你今天心情不错我很开心。"
- 不要整段只有韩语没有中文翻译，每条回复都必须附带中文翻译部分。
- 中文翻译部分用【中文翻译】标记开头，放在回复末尾。
- 韩语练习模式(korean)可以更详细解释韩语语法和词汇。
- 其他模式(daily/comfort/study/romantic)也必须用韩语回复，只是语气和风格按模式调整。
`;
export async function getAIResponse(memberId, userMessage, chatHistory, mode = 'daily', mood = 'normal', nickname = '朋友', relationshipTitle = '初识粉丝') {
...
function buildSystemPrompt(member, mode, mood, nickname, relationshipTitle) {
...
function getFallbackResponse(memberKey) {
