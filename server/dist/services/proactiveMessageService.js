...
export function generateProactiveMessages(userId, memberId) {
    const member = db.prepare('SELECT member_key FROM members WHERE id = ?').get(memberId);
    if (!member)
        return [];
...
export function getUnreadProactiveMessages(userId, memberId) {
    return db.prepare(`
...
export function getAllUnreadProactiveMessages(userId) {
    return db.prepare(`
...
export function markProactiveMessagesAsRead(userId, memberId) {
    db.prepare(`
    UPDATE proactive_messages SET is_read = 1 WHERE user_id = ? AND member_id = ? AND is_read = 0
  `).run(userId, memberId);
}
export function addIntimacyUpgradeMessage(userId, memberId, newTitle) {
    const member = db.prepare('SELECT member_key FROM members WHERE id = ?').get(memberId);
    if (!member)
        return '';
    const templates = PROACTIVE_MESSAGE_TEMPLATES[member.member_key];
    if (!templates || !templates.intimacy_upgrade)
        return '';
