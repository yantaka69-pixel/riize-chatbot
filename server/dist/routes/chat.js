...
import { markProactiveMessagesAsRead, addIntimacyUpgradeMessage } from '../services/proactiveMessageService.js';
const router = Router();
...
router.post('/message', async (req, res) => {
    try {
        ...
        // Verify user exists
        const user = db.prepare('SELECT id, nickname, current_mood FROM users WHERE id = ?').get(userId);
        if (!user) {
            return res.status(404).json({ error: '用户不存在' });
        }
        // Verify member exists
        const member = db.prepare('SELECT id, member_key FROM members WHERE id = ?').get(memberId);
        if (!member) {
    ...
    `).run(uuidv4(), userId, memberId, content, mode);
        ...
        const aiResult = await getAIResponse(memberId, content, chatHistory, mode, moodToUse, nickname, relationshipTitle);
        // Save AI response
