...
router.post('/verify-password', (req, res) => {
  try {
    ...
    const settings = db.prepare('SELECT admin_password_hash FROM settings WHERE id = ?').get('settings_1') as any;
...
router.get('/api-config', (req, res) => {
  try {
    const settings = db.prepare('SELECT base_url, model_name FROM settings WHERE id = ?').get('settings_1') as any;
